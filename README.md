...
Correctness, measured row for row against the real KQL engine (the Kusto
Emulator). These are conformance numbers, not performance ones — the emulator is
used only as a correctness oracle
([scope of use](https://github.com/mmaitre314/duckdb-kql/blob/main/docs/oracle-harness.md#scope-of-use)).
...
(Apache-2.0, vendored at a pinned commit with local patches marked in-file — see [provenance](https://github.com/mmaitre314/duckdb-kql/blob/main/grammar/UPSTREAM.md)).
...
see [`docs/oracle-harness.md`](https://github.com/mmaitre314/duckdb-kql/blob/main/docs/oracle-harness.md). It is used only as a
correctness oracle: a development and CI tool, neither a runtime dependency nor
redistributed, and no performance figure is taken from it or published
([scope of use](https://github.com/mmaitre314/duckdb-kql/blob/main/docs/oracle-harness.md#scope-of-use)).
...
It vendors an Apache-2.0 grammar, harvests MIT-licensed documentation samples,
reimplements an MIT-licensed SDK interface, and outlines its wordmark from a
Bitstream Vera-licensed typeface. Full license texts are in
[`licenses/`](https://github.com/mmaitre314/duckdb-kql/tree/main/licenses), shipped in the wheel and the sdist rather

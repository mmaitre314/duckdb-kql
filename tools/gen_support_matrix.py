...
def build() -> str:
    ...
    a("")
    a("`between` and `!between` are supported. `x between (low .. high)` is")
    a("**inclusive at both ends** and **null whenever any of the three is null**")
    a("— the value or either bound — which is what SQL `BETWEEN` already does, so")
    a("this is one operator where the obvious emission is the measured one. A")
    a("reversed range is simply false, not an error.")
    a("")
    a("The trap is a **timespan high bound**, which is a *duration*:")
    a("`t between (datetime(2020-01-01) .. 3d)` ends at 2020-01-04 inclusive,")
    a("while `d between (2h .. 6h)` over a timespan column is the plain reading.")
    a("The completion is applied only where both bounds are statically typed; a")
    a("bare column carries no type, so anything less falls through to the plain")
    a("reading — where that is the wrong guess the operands are a TIMESTAMP and")
    a("an INTERVAL and DuckDB refuses them, so a missed completion costs an error")
    a("rather than rows from the wrong window.")
    a("")
    a("Kusto takes only *ordered* scalars here — numeric, `datetime`, `timespan`")
    a("and `bool` — and refuses `string`, `dynamic` and `guid` with SEM0208. So")
    a("does this, for a literal operand; a bare string column still answers,")
    a("because nothing here knows it is a string.")
    a("")

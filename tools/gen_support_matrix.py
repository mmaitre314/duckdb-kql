...
RULES = {
    ...
    "R19": "`parse` is **all-or-nothing**: one failed conversion blanks the whole row. A non-match keeps the row with `''`, not null.",
}
...
OPERATORS: list[tuple[str, str, str]] = [
    ...
    ("parse", "T | parse a with 'x' v", "`kind=simple` (the default) and `kind=relaxed`. **All-or-nothing**: under `simple`, one failed conversion blanks the *whole row*, columns that converted included. A non-match keeps the row with `\'\'` for a string column and null for a typed one. A capture is lazy up to the next literal, except immediately before a `*`, where a typed column matches its type\'s shape and a string column is refused as ambiguous (SEM0476, as Kusto refuses it). Conversions are KQL\'s, not DuckDB\'s — `\'1.5\'` as a long is null, not 2. Not supported: `kind=regex`, `flags=`, `: decimal`, and a `datetime`/`timespan` column before a `*`. See R19."),
